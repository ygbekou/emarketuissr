...
import { ServiceDescription } from 'src/app/app.models';
...
export class ServicesComponent extends BaseComponent implements OnInit {
  ...
  @ViewChild(ServiceComponent, { static: false }) serviceView: ServiceComponent;

  displayedColumns: string[] = ['name', 'status', 'actions'];
  dataSource: MatTableDataSource<ServiceDescription>;
  ...
  public remove(sDesc: ServiceDescription) {
    this.messages = '';
    this.appService.delete(sDesc.service.id, 'Service')
      .subscribe(resp => {
        if (resp.result === 'SUCCESS') {
          const index: number = this.dataSource.data.indexOf(sDesc);
          if (index !== -1) {
            this.dataSource.data.splice(index, 1);
            this.dataSource = new MatTableDataSource<ServiceDescription>(this.dataSource.data);
            this.dataSource.paginator = this.paginator;
            this.dataSource.sort = this.sort;
          }
  ...
  onSave($event) {
    ...
    sce.serviceDescriptions.forEach(element => {
        if (element.language.id === this.appService.appInfoStorage.language.id) {
          sce.serviceDescriptions[0].service = sce;
          if (!this.dataSource.data) {
            this.dataSource.data = [];
          }
          this.updateDatasourceData(this.dataSource, this.paginator, this.sort, sce.serviceDescriptions[0]);
        }
    });
  }
}
